    ...
    def generate_final_answer_node(self, state: GraphState) -> Dict[str, Any]:
        ...
        for qa_item in qa_results:
            ...
            for data_source in qa_item["data"]:
                ...
                formatted_qa_for_final_answer += f"--- Источник {source_counter} (поисковый запрос: '{str(qa_item['original_search_query_context'])}') ---\n"
                formatted_qa_for_final_answer += f"Заголовок: {str(data_source['title'])}\n"
                formatted_qa_for_final_answer += f"Ссылка: {str(data_source['url'])}\n"
                formatted_qa_for_final_answer += f"Краткий ответ по этому источнику: {str(qa_item['answer'])}\n"
                formatted_qa_for_final_answer += f"Фргамент текста, на базе которого сформулирован краткий ответ: {str(data_source['fragment'])}\n\n"
                source_counter += 1
