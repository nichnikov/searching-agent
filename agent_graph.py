...
from searchers.combined_web_searcher import CombinedWebSearcher, MultiQuerySearcher
...
class LangGraphPipeline:
    ...
    def __init__(self, llm_handler: LLMHandler, web_searcher: CombinedWebSearcher, max_retries: int = 2):
        ...
        # ИЗМЕНЕНИЕ: Создаем экземпляр MultiQuerySearcher для использования в узлах
    ...
    def web_search_node(self, state: GraphState) -> Dict[str, Any]:
        """
        Выполняет поиск в вебе по сгенерированным запросам, используя MultiQuerySearcher.
        """
        ...
        # ИЗМЕНЕНИЕ: Заменяем цикл одним вызовом MultiQuerySearcher
        new_results = self.multi_searcher.search_all(
            queries=queries, 
            num_results=config.DEFAULT_LIMIT
        )
        print(f"Мульти-поиск дал {len(new_results)} уникальных результатов.")

        # Корректно объединяем старые и новые результаты, избегая дубликатов
        final_results = list(previous_results)
        seen_urls = {res.get('url') for res in previous_results if res.get('url')}
        ...
        print(f"Общее количество уникальных результатов после объединения: {len(final_results)}.")
    ...
    def analyze_results_node(self, state: GraphState) -> Dict[str, Any]:
        ...
        # Анализируем только те результаты, которые еще не были проанализированы
        # Это важно, так как search_results теперь накапливаются
        already_relevant_count = len(state.get('relevant_documents', []))
        all_search_results = state.get('search_results', [])
        
        # Определяем срез новых результатов для анализа.
        # Это более надежный способ, чем расчет по длине.
        previously_analyzed_urls = {doc.get('url') for doc in state.get('relevant_documents', [])}
        results_to_analyze = [res for res in all_search_results if res.get('url') not in previously_analyzed_urls]

        if not results_to_analyze:
             print("Новых результатов для анализа нет. Завершаю...")
             return {"feedback": "Поиск не дал новых результатов."}
        ...
        try:
            ...
            return {"feedback": "Ошибка анализа результатов."}
    ...
    def run(self, query: str):
        initial_state = GraphState(
            original_query=query, search_queries=[], search_results=[],
            relevant_documents=[], feedback="", rephrasing_count=0, final_answer=""
        )
...
# --- Фабрика и Main (без изменений) ---
# ... (код ComponentFactory и main() остается прежним)
